...
public class User {
	...
	public boolean setEmail(String email) {
		...
		boolean valid = true;
		/*
		 * will be flagged false if at least one condition below for invalidity
		 * is met
		 */
		...
		else if (email.substring(email.indexOf(".") + 1).length() < 2
				|| email.substring(email.indexOf(".") + 1).length() > 4) {
			valid = false;
		}
	...
	public boolean setPassword(String password) {
		...
		if (pass_char.length < 9) {
			...
			boolean lower = false;
			// lastly, loop through the characters and make sure at least 4
			// digits
			int digits = 0;
