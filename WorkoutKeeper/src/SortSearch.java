 ...
 *         contains methods that handle searching and sorting tasks accomplished
 *         throughout the program.
 */
public class SortSearch {
	/**
	 * Reads the logged-in user's exercises from a file and returns them as objects
	 * stored in a linked list
	 * 
	 * @param user
	 *            The logged in user
	 * @param counter
	 *            the number of stretch, cardio, and weight training exercises
	 * 
	...
	/**
	 * Creates the appropriate exercise object (Cardiovascular, Stretch,
	 * WeightTraining) and returns it
	 * 
	 * @param record
	 *            The String line to parse for object fields
	 * 
	 * @param counter
	 *            A counter to keep track of number of exercise objects instantiated
	 * 
	...
	/**
	 * Sorts the exercises stored in the input linked list based on input sorting
	 * method
	 * 
	 * @param list
	 *            A linked list containing all of the user's exercises
	 * 
	 * @param howSort
	 *            The sorting method as a String (type, muscle group, duration)
	 * 
	 * @return display String made by concatenating toString() returns of
	 *         appropriate exercise objects
	 */
	public static String sortingMethod(LinkedList<Exercise> list, String howSort) {
		String display = "";
		// for ease of sorting, turn linked list into array (length is 1 more than last
		// element index)
		...
		if (howSort.equalsIgnoreCase("Type")) {
			...
			// concatenate list of cardio exercises first, then stretches, then weight
			// training
			display += cardio + "\n" + stretch + "\n" + training;
		}
		// concatenate toString() returns of appropriate exercise objects by muscle
		// group: bucket sort
			...
			// holding linked lists in each cell to hold exercises toString() that fall
			// under each muscle group
			ArrayList<LinkedList<String>> list_muscle = new ArrayList<LinkedList<String>>();
			// add the number of buckets (linked list) as there are muscle groups to the
			// array
			for (int x = 0; x < Exercise.MUSCLE_GROUP.length; x++) {
				list_muscle.add(new LinkedList<String>());
			}
			// iterate through input linked list, which contains all exercises belonging to
			// the logged-in user
			...
			// once input list is finished being processed, proceed to concatenate all
			// toString() to display
	...
	/**
	 * Sorts the input sublist of cardiovascular exercises belonging to one user by
	 * duration
	 * 
	 * @param subList
	 *            A sub-linked list containing only cardio exercises belonging to
	 *            one user
	 * 
	 * @return display A String created from concatenating information across all
	 *         cardio exercises
	 */
	public static String sortDuration(LinkedList<Exercise> subList) {
		...
		// add each item in the sublist of cardio exercises into the tree, which will
		// sort by duration automatically
	...
	/**
	 * Returns a linked list (sublist of input linked list) of a specific type of
	 * exercise
	 * 
	 * @param list
	 *            A linked list of all of the logged-in user's exercises
	 * 
	 * @param type
	 *            The type of exercises to filter out
	 * 
	 * @return subList A sublist of the input list containing a specific type of
	 *         exercise
	 */
	public static LinkedList<Exercise> returnTypeList(LinkedList<Exercise> list, String type) {
		...
		// iterate through the linked list and extract every record that falls under the
		// input type
		Iterator<Exercise> it = list.iterator();
		while (it.hasNext()) {
			Exercise current = (Exercise) it.next();
	...
	/**
	 * Searches the cardiovascular exercises belonging to the user based on a input
	 * duration
	 * 
	 * @param list
	 *            A linked list of all of the logged-in user's exercises
	 * 
	 * @param duration
	 *            The input duration to search for
	 * 
	 * @return display The display of results by concatenating the toString() of all
